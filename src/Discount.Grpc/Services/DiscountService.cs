...
namespace Discount.Grpc.Services
{
    public class DiscountService : DiscountProtoService.DiscountProtoServiceBase
    {
        ...
        public override async Task<CouponRequest> GetDiscount(GetDiscountRequest request, ServerCallContext context)
        {
            ...
            _logger.LogInformation("Discount is retrived for ProductName :{poductName},Amount : {amount}", coupon.PoductName, coupon.Amount);
        ...
        public override async Task<CouponRequest> CreateDiscount(CouponRequest request, ServerCallContext context)
        {
            ...
            if (isSaved)
            {
                _logger.LogInformation("Discount is successfully created. ProductName :{ProductName}", coupon.PoductName);
            }
            else
            {
        ...
        public override async Task<CouponRequest> UpdateDiscount(CouponRequest request, ServerCallContext context)
        {
            ...
            if (IsModified)
            {
                _logger.LogInformation("Discount is successfully updated. ProductName : {ProductName}", coupon.PoductName);
            }
            else
            {
